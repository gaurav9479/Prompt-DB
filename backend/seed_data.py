...
async def seed_shop_categories():
    ...
    categories = [
        ...
        {"name": "Dairy Products", "icon": "🥛", "description": "Fresh milk, curd, cheese, and butter", "is_perishable": True},
    ]
    ...
    shops = [
        {
            ...
            "owner_email": "admin@promptdb.com",
...
async def seed_products(shop_ids, category_ids):
    ...
    shop_products = {
        "TechHub Electronics": electronics_products,
    }
...
async def seed_customers():
    first_names = [
        "Aarav", "Aditi", "Arjun", "Ananya", "Kabir", "Diya", "Rohan", "Isha", "Vivaan", "Meera",
        "Reyansh", "Sneha", "Vihaan", "Tanvi", "Krishna", "Riya", "Sai", "Kiara", "Aaryan", "Pooja",
        "Dev", "Neha", "Ishan", "Shruti", "Rahul", "Priya", "Vikram", "Shreya", "Sanjay", "Nisha",
        "Amit", "Geeta", "Rajesh", "Sunita", "Anil", "Kavita", "Sunil", "Aarti", "Vijay", "Jyoti",
        "Ramesh", "Rekha", "Suresh", "Anjali", "Harish", "Pinky", "Dinesh", "Ritu", "Manoj", "Sapna"
    ]
    last_names = [
        "Patel", "Sharma", "Verma", "Gupta", "Iyer", "Rao", "Nair", "Mehta", "Joshi", "Sen",
        "Das", "Kulkarni", "Deshmukh", "Bhat", "Reddy", "Choudhury", "Banerjee", "Chatterjee", "Mukherjee", "Roy",
        "Singh", "Yadav", "Mishra", "Prasad", "Pandey", "Shrivastav", "Saxena", "Kapoor", "Khanna", "Malhotra",
        "Sethi", "Gill", "Sodhi", "Ahluwalia", "Grewal", "Dhillon", "Chawla", "Grover", "Anand", "Kumar",
        "Lal", "Shah", "Patil", "Naik", "Gowda", "Hegde", "Menon", "Pillai", "Dutt", "Dutta"
    ]

    customers = [
        {"name": "Test Customer", "email": "customer@promptdb.com", "phone": "+91 98765 43210", "address": "123 MG Road, Mumbai 400001"},
    ]

    used_emails = {"customer@promptdb.com"}
    ...
    while len(customers) < 50:
        ...
        address = f"{random.randint(1, 999)}, {random.choice(['MG Road', 'Indiranagar', 'Koramangala', 'HSR Layout', 'Whitefield', 'Jayanagar'])}, Bangalore {random.randint(560001, 560100)}"
        customers.append({
            "name": name,
            "email": email,
            "phone": phone,
    ...
    async with async_session() as db:
        ...
        result = await db.execute(text("SELECT name, id, email, phone, address FROM customers"))
        return {row[0]: (row[1], row[2], row[3], row[4]) for row in result.fetchall()}
...
async def seed_users(shop_ids):
    ...
    users = [
        {"name": "Platform Admin", "email": "superadmin@promptdb.com", "role": "super_admin", "shop_id": None},
        {"name": "Rahul Verma", "email": "admin@promptdb.com", "role": "admin", "shop_id": shop_ids.get("TechHub Electronics")},
        {"name": "Test Customer", "email": "customer@promptdb.com", "role": "customer", "shop_id": None},
    ]
...
async def seed_orders(products_data, customer_ids):
    statuses = ["delivered", "delivered", "delivered", "shipped", "confirmed", "pending"]
    ...
    async with async_session() as db:
        ...
        # Seed 350 orders spread over the last 90 days
        for i in range(350):
            product = random.choice(products_data)
            product_id, product_name, price, cost_price, shop_id = product
            
            customer_name, (customer_id, customer_email, customer_phone, customer_address) = random.choice(customer_list)
            qty = random.randint(1, 3)

            # Some bargaining/discount variation
            bargain_discount = random.choice([0, 0, 0, 0, 0.05, 0.1, 0.15, 0.2])
            ...
            # Spread orders over the last 90 days. We can make it slightly trend upwards.
            # E.g., distribute i from 0 to 350 to spread the days.
            # Day diff from 90 down to 0
            day_diff = int(90 - (i / 350.0) * 90) + random.randint(-2, 2)
            day_diff = max(0, min(90, day_diff))
            
            created_at = datetime.now() - timedelta(days=day_diff, hours=random.randint(0, 23), minutes=random.randint(0, 59))
            ...
            if customer_id not in customer_stats:
                customer_stats[customer_id] = {"orders": 0, "spent": 0.0}
            customer_stats[customer_id]["orders"] += 1
            customer_stats[customer_id]["spent"] += total_amount

            if shop_id not in shop_stats:
                shop_stats[shop_id] = {"orders": 0, "revenue": 0.0}
            shop_stats[shop_id]["orders"] += 1
            shop_stats[shop_id]["revenue"] += total_amount
        ...
        for cust_id, stats in customer_stats.items():
            await db.execute(
                text("UPDATE customers SET total_orders = :orders, total_spent = :spent WHERE id = :id"),
                {"orders": stats["orders"], "spent": stats["spent"], "id": cust_id}
            )
        
        # Update shop stats in DB
        for sh_id, stats in shop_stats.items():
            await db.execute(
                text("UPDATE shops SET total_orders = :orders, total_revenue = :revenue WHERE id = :id"),
                {"orders": stats["orders"], "revenue": stats["revenue"], "id": sh_id}
            )
        
        await db.commit()
        print("✓ Synchronized customer and shop total stats with generated orders")


async def main():
    print("\n🌱 Seeding Prompt-DB Database with Comprehensive Data...\n")
